"""Celery task: `match.process`. Session/transaction lifecycle only — the
actual logic lives in app/services/match_service.py, same split as
app/workers/background.py / app/services/background_service.py.
...
`asyncio.wait_for`, same as background.py/generation.py.

Unlike background.py: no QA dispatch after a successful run. MATCH ships
straight to COMPLETED on a successful provider call (see
app/services/match_service.py's module docstring) — there's nothing further
to dispatch.
"""
...
from app.services.match_service import process
...
async def _run_timed_out(sub_job_id: str) -> str:
    ...
    already operation-agnostic (keyed on sub-job status, not
    angle/background/match), the same way status_rollup.compute_parent_status
    is.
    """
    engine = create_async_engine(settings.DATABASE_URL)
    try:
...
@celery_app.task(name="match.process")  # type: ignore[untyped-decorator]
