"""Celery task: `mix.process`. Session/transaction lifecycle only — the
actual logic lives in app/services/mix_service.py, same split as
app/workers/recolor.py / app/workers/match.py.
...
`asyncio.wait_for`, same as recolor.py/background.py/match.py/generation.py.

Unlike background.py: no QA dispatch after a successful run. MIX ships
straight to COMPLETED on a successful generate-then-composite (see
app/services/mix_service.py's module docstring) — there's nothing further
to dispatch.
"""
...
from app.services.mix_service import process
...
async def _run_timed_out(sub_job_id: str) -> str:
    """See recolor.py's/background.py's `_run_timed_out` — same fresh-session
    reasoning. Reuses generation_service.mark_sub_job_timed_out unmodified:
    it's already operation-agnostic, the same way
    status_rollup.compute_parent_status is."""
    engine = create_async_engine(settings.DATABASE_URL)
    try:
        factory = async_sessionmaker(engine, expire_on_commit=False)
        async with factory() as session:
...
@celery_app.task(name="mix.process")  # type: ignore[untyped-decorator]
