...
Migration `0005_asset_purged_at.py` (chained off `0003`; a sibling Phase 3
agent is independently adding `0004` in a different worktree — the
`0003 -> 0004 -> 0005` chain gets reconciled at merge time, not here): adds
`assets.purged_at TIMESTAMPTZ NULL`. Needed because "storage lifecycle
removes bytes" (`docs/business-rules.md` §11) has to know which expired
rows it has already swept, or a beat task run every few hours would
...
- [x] `alembic upgrade head` applies `0005` cleanly on top of `0003`;
      `downgrade()` to `0003` reverses it — both verified by
      `tests/integration/test_migrations.py::test_0005_adds_and_removes_assets_purged_at`
